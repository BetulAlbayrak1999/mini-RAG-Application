...
class QdrantDBProvider(VectorDBInterface):
    ...
    def create_collection(
        ...
        if not self.client.create_collection(collection_name=collection_name):
    ...
    def insert_one(
        ...
        if not self.is_collection_existed(collection_name=collection_name):
            self.logger.error(
                f"Can not insert new record to non-existed collections: {collection_name}"
            )
            return False
        try:
    ...
    def insert_many(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            batch_vector_ids = record_ids[i:batch_end]

            batch_records = [
                models.Record(
                    id=batch_vector_ids[x],
