...
class PGVectorProvider(VectorDBInterface):
    ...
    async def reset_vector_index(
        ...
        async with self.db_client() as session:
            async with session.begin():
                drop_sql = sql_text(f"DROP INDEX IF EXISTES {index_name}")
            ...
            async with session.begin():
                ...
                for i in range(0, len(texts), batch_size):
                    ...
                    for _text, _vector, _metadata, _record_id in zip(
                        ...
                        flat_vector = _vector[0] if isinstance(_vector, list) and len(_vector) == 1 and isinstance(_vector[0], list) else _vector

                        values.append(
                            {
                                "text": _text,
                                "vector": "[" + ",".join([str(v) for v in flat_vector]) + "]",
                                "metadata": metadata_json,
                                "chunk_id": _record_id,
                            }
                        )
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int):
        ...
        if not is_collection_existed:
            self.logger.error(
                f"Can not search for records in non-existed collection: {collection_name}"
            )
            return False
        
        # Handle nested vector arrays (flatten if needed)
        ...
        vector_str= "["+ ",".join([str(v) for v in flat_vector]) + "]"
